...
# 이벤트 정의
# RD, LD, RU, LU = 0, 1, 2, 3
RD, LD, RU, LU, TIMER, DA = range(6) # 위에꺼와 동일

# 키 입력을 단순화 시켜서 이벤트로 해석
key_event_table = {
    (SDL_KEYDOWN, SDLK_RIGHT): RD,
    (SDL_KEYDOWN, SDLK_LEFT): LD,
    (SDL_KEYUP, SDLK_RIGHT): RU,
    (SDL_KEYUP, SDLK_LEFT): LU,
    (SDL_KEYDOWN, SDLK_a): DA
}
...
class AUTO_RUN:
    ...
    def draw(self):
        if self.dir == -1:
            self.image.clip_draw(self.frame*100, 0, 100, 100, self.x, self.y,150,150)
        elif self.dir == 1:
            self.image.clip_draw(self.frame*100, 100, 100, 100, self.x, self.y,150,150)
class SLEEP:
    def enter(self, event): # 상태에 들어갈 때 행하는 액션
        print('ENTER IDLE')
        pass
...
next_state = {
    ...
    IDLE:   {RU: RUN, LU: RUN, RD: RUN, LD: RUN, TIMER: SLEEP, DA: AUTO_RUN},
    RUN:    {RU: IDLE, LU: IDLE, RD: IDLE, LD: IDLE, DA: AUTO_RUN},
    AUTO_RUN: {RD: RUN, LD: RUN, RU: RUN, LU: RUN, DA: IDLE}
}
